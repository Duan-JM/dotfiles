...
- **闸门后才写预期差**：只有粗读结论为 `观察池 / 进入深研` 时，才启用预期差验证框架；
  `排除 / 信息不足` 不得被强制扩写。
- **预期差可证伪**：市场隐含预期、我方预期、证据锚点、差异方向、验证日期、先行指标、
  上 / 下行失效条件必须写清；并分离内在价值判断、1-3个月路径判断与研究动作。
...
## expectation_gap_after_gate
- 启用状态：`启用 / 不启用`（仅当粗读结论倾向为 `观察池 / 进入深研` 时启用）
- 不启用原因：价格闸门未过 / 硬伤命中 / 证据不足 / 无独立观点等
- 市场隐含预期：当前价格、估值、市场叙事或事件定价暗含了什么
- 我方预期：与市场不同且可检验的判断；没有独立观点写"暂无可证伪预期差"
- 证据锚点：Fact ID / SRC-XXX / 前置章节线索
- 差异方向：`市场过悲观 / 市场过乐观 / 市场忽视关键变量 / 信息不足`
- 验证日期：YYYY-MM-DD 或下一次财报 / 公告 / 行业数据窗口
- 先行指标：3-5 个可提前观察的指标
- 上行失效条件 / 下行失效条件：分别说明什么证据会否定上行预期差或下行风险判断
- 判断分层：内在价值判断 / 1-3个月路径判断 / 研究动作（研究动作只能是排除、观察、
  继续验证、进入深研等资源分配，不得是买入 / 卖出 / 持有）
...
- 08_investment_thesis：本公司的关键观察指标（5-10 个 KPI）与预期差验证重点
...
6. `09_research_decision.md` — 是否进入深研
   ...
   - 只有结论为 `观察池 / 进入深研` 时，才在闸门之后加入预期差验证框架；`排除 / 信息不足`
     不强制扩写
   - 400-800 字
...
8. `08_investment_thesis.md` — 投资逻辑与风险
   ...
   - 必须包含可证伪预期差与判断分层；无独立观点时研究动作降为"观察 / 继续验证"
   - 700-1200 字
...
short / deep 第 `08` 章必须包含可证伪预期差；若没有独立观点，研究动作降为"观察 / 继续验证"，
不得输出买卖建议。
...
| `S5` | style | 引用财报质量核查 JSON 时，遗漏 `errors` / `missing_fields`，或把 `not_calculated` 写成已通过 | patch |
| `S6` | style | 预期差结构缺失或不可证伪；08 章缺市场隐含预期 / 我方预期 / 证据锚点 / 差异方向 / 验证日期 / 先行指标 / 上下行失效条件，或缺内在价值 / 1-3个月路径 / 研究动作三分法 | patch / regenerate |
| `S7` | style | earnings 模式缺首次覆盖基线，或旧预测未保留原文 / 状态不合法 | patch |
...
| `S6` 预期差结构缺失 | 在既有证据内补齐；无独立观点则写"暂无可证伪预期差"并把研究动作降为"观察 / 继续验证" |
| `S7` earnings 基线违规 | 保留原预测、补首次覆盖基线或把状态改为 `命中 / 部分命中 / 未命中 / 无法验证` 之一 |
...
- short / deep 第 08 章是否包含可证伪预期差；无独立观点时是否降级为观察 / 继续验证；
  第 09 章是否分离内在价值判断、1-3个月路径判断与研究动作
- earnings 模式是否保持 10-18 九段式结构、首次覆盖基线 / 旧预测状态一致，且不输出评级或买卖建议
