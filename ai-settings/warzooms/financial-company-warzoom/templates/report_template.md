...
如果 `company.md` 中 `报告模式=earnings`，只沉淀财报点评所需事实，不生成实际报告：
- 本期财报公告、业绩材料、电话会 / 指引、公司旧指引与用户旧预测
- 分部 KPI、利润率、费用率、现金流、资本开支、分红回购、现金与债务
- 市场预期、同业财报与财报发布后 1-3 个交易日市场反应
...
## earnings 模式：九段式财报点评（观察池 / 持续跟踪）

earnings 模式使用独立章节 `10`-`18`，不改变 rough 默认。所有章节必须先判断基线：
无历史预测时写"首次覆盖基线"；有旧预测时逐条保留原预测文本，验证状态只能使用
`命中 / 部分命中 / 未命中 / 无法验证`。

---CHAPTER_10_PROMPT---
你是财报跟踪负责人。请撰写"十、财报结论快照"。
...
要求：
- 一句话结论：本期财报是否提高 / 降低 / 不改变观察池跟踪优先级。
- 固定列出：财报期、披露日期、收入 / 利润 / 现金流核心结果、观察池状态变化。
- 列出 3 个以内最关键的超预期 / 低预期点。
- 明确写"首次覆盖基线"或"基于旧预测复盘"；不得把首次覆盖写成命中 / 未命中。
- 所有定量数字必须内联引用 `SRC-XXX`。
---END---

---CHAPTER_11_PROMPT---
你是预期差与财报质量审计角色。请撰写"十一、预期差与财报质量"。
...
- 有旧预测时必须用表格保留 `原预测 / 预测来源日期 / 实际结果 / 验证状态 / 证据`。
- 验证状态只能为：`命中 / 部分命中 / 未命中 / 无法验证`。
- 无历史基线时明确写"本次为首次覆盖基线，不做命中率归因"，并给出未来基线指标。
- 评价财报质量：一次性项目、会计口径变化、递延 / 提前确认、非经营性收益。
---END---
...
要求：
- 按分部 / 地区 / 产品列出收入、增速、毛利率或业务 KPI。
- KPI 可包括订单、销量、价格、用户数、ARPU、产能利用率、门店数等，按行业自适应。
- 标出本期变化真正来自价格、销量、结构、汇率、并购还是会计口径。
- 缺失项写"暂未获取"，不得估算补齐。
---END---
...
要求：
- 拆解毛利率、经营利润率、净利率变化。
- 分析研发 / 销售 / 管理 / 财务费用率及一次性项目。
- 说明利润改善或恶化是经营杠杆、价格、成本、产品结构还是费用节奏驱动。
- 所有数字用表格列示并带 `SRC-XXX`。
---END---
...
要求：
- 覆盖经营现金流、自由现金流、营运资本、资本开支、现金、债务。
- 覆盖分红、回购、增发、股权激励摊薄或债务再融资。
- 判断利润是否转化为现金，并列出下一期需跟踪的现金流缺口。
---END---
...
要求：
- 摘要管理层定量 / 定性指引、口径变化和电话会问答增量信息。
- 区分已披露事实、管理层指引和研究者假设。
- 列出 3-5 个下一季必须验证的问题及对应证据类型。
---END---
...
要求：
- 对照同业财报或行业数据，说明本期表现是公司特异还是行业共振。
- 记录财报发布后 1-3 个交易日股价、成交量、估值倍数或信用利差变化。
- 只解释市场反应，不输出买卖建议或评级。
---END---
...
前序章节：{previous_sections}

要求：
- 用变化桥说明旧模型到新模型的收入、毛利率、费用、现金流、资本开支、估值倍数变化。
- 有旧预测时保留旧预测，不得事后改写；无旧模型时建立"首次覆盖模型基线"。
- 明确哪些变化来自事实更新，哪些只是待验证假设。
- 不给目标价、评级或买卖建议。
---END---
...
要求：
- 更新多头论点、空头论点、关键变量与观察池优先级。
- 给出行动清单：继续跟踪 / 降低优先级 / 进入深研 / 信息不足，并说明触发条件。
- 列出下一期跟踪 KPI、负责人可执行的证据获取路径和时间点。
- 末尾固定附免责声明：本报告基于公开信息整理，不构成投资建议。
---END---
