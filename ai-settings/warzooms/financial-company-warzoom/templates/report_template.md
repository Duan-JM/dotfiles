...
- **可证伪预期差框架**（short / deep 必须；rough 模式默认不生成本章）：
  | 市场隐含预期 | 我方预期 | 证据锚点 | 差异方向 | 验证日期 | 先行指标 | 上行失效条件 | 下行失效条件 |
  |--------------|----------|----------|----------|----------|----------|--------------|--------------|
  | 用当前估值、价格行为、卖方/公告口径或行业共识概括 | 写我方与市场不同的可检验判断；没有独立观点写"暂无" | SRC-XXX / 前文章节 | 市场过悲观 / 过乐观 / 信息不足 | YYYY-MM-DD 或事件窗口 | 1-3 个可追踪指标 | 什么证据会否定上行判断 | 什么证据会否定下行风险 |
- **判断分层**：必须分开写「内在价值判断」「1-3个月路径判断」「研究动作」。研究动作只能是
  `排除 / 观察 / 继续验证 / 进入深研` 等研究资源分配，不得写成买入 / 卖出 / 持有建议。
- 投资初筛必须包含：
...
要求：
- 仅做事实推导，**不给买入/卖出/持有评级**
- short / deep 投资论点必须包含可证伪预期差；若无法形成独立于市场隐含预期的观点，
  明确写"暂无可证伪预期差"，并将研究动作降为"观察 / 继续验证"，不得给买卖建议。
...
- 只有研究结论为 `观察池` 或 `进入深研` 时，才在闸门表之后追加"预期差验证框架"；
  `排除` 或 `信息不足` 时不得为了完整性扩写，只列缺口与停止 / 观察理由。
- 若启用"预期差验证框架"，必须使用固定字段：
  | 市场隐含预期 | 我方预期 | 证据锚点 | 差异方向 | 验证日期 | 先行指标 | 上行失效条件 | 下行失效条件 |
  |--------------|----------|----------|----------|----------|----------|--------------|--------------|
  同时分开写：
  - 内在价值判断：是否存在可识别的底、内在价值锚或安全边际缺口
  - 1-3个月路径判断：短期价格 / 事件路径是否可判断；不可判断就写"暂不判断"
  - 研究动作：排除 / 观察 / 继续验证 / 进入深研，不得写买入 / 卖出 / 持有
...
- short / deep 启用本章时，若没有独立观点或关键证据不足，研究动作降为"观察 / 继续验证"。
---END---
...
- rough 模式：
  ...
  - 最后列 3 条以内下一步验证问题。
  - 仅当初步结论为 `观察池` 或 `进入深研` 时，在闸门表后增加极简"闸门后预期差假设"：
    市场隐含预期 / 我方预期 / 证据锚点 / 差异方向 / 验证日期 / 先行指标 / 上行失效条件 /
    下行失效条件。若结论为 `排除` 或 `信息不足`，不得强制扩写该框架。
