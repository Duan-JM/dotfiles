...
- 若粗读闸门倾向为 `观察池` 或 `进入深研`，给出闸门之后的预期差假设；若为
  `排除` 或 `信息不足`，只写未启用原因，避免 rough 模式被强制扩写
...
- 预期差必须可证伪：区分市场隐含预期与我方预期，写清证据锚点、方向、验证日期、
  先行指标与上 / 下行失效条件；若没有独立观点，写"暂无可证伪预期差"。
...
- 启用状态：启用 / 不启用（仅当粗读结论倾向为观察池或进入深研时启用；排除 / 信息不足时不启用）
- 不启用原因：<若不启用，写价格闸门未过 / 硬伤命中 / 证据不足 / 无独立观点等>
- 市场隐含预期：<当前价格、估值、市场叙事或事件定价暗含了什么；证据不足写暂未获取>
- 我方预期：<我方与市场不同且可检验的判断；无独立观点写"暂无可证伪预期差">
- 证据锚点：<Fact ID / SRC-XXX / 前置章节线索；不得新增未登记来源>
- 差异方向：市场过悲观 / 市场过乐观 / 市场忽视关键变量 / 信息不足
- 验证日期：<YYYY-MM-DD 或下一次财报 / 公告 / 行业数据窗口>
- 先行指标：<3-5 个可提前观察的指标>
- 上行失效条件：<什么证据会否定上行预期差>
- 下行失效条件：<什么证据会否定下行风险判断>
- 判断分层：
  - 内在价值判断：<底、价值锚、安全边际是否成立>
  - 1-3个月路径判断：<短期路径是否可判断；不可判断写暂不判断>
...
- 07_swot：<本公司天然非对称视角>
- 08_investment_thesis：<本公司的 5-10 个 KPI + 预期差验证重点；若无独立观点则提示研究动作降为观察 / 继续验证>
