...
- 按下列动作映射执行：
  ...
  - `S6` 预期差结构缺失 → 仅在已有证据边界内补齐市场隐含预期、我方预期、证据锚点、
    差异方向、验证日期、先行指标、上行失效条件、下行失效条件，以及内在价值判断 /
    1-3个月路径判断 / 研究动作；若无独立观点，写"暂无可证伪预期差"，研究动作改为
    "观察 / 继续验证"，不得写买入 / 卖出 / 持有
  - `S7` earnings 基线违规 → 补"首次覆盖基线"，或保留旧预测原文并将验证状态限定为 `命中 / 部分命中 / 未命中 / 无法验证`
