...
> **用途**：仅在结构性失败（`S3` / `S4` / `S6`）或大段 evidence 失败时，基于章节骨架与修复合同重建整章。
...
- 仅在结构性失败（`S3` / `S4` / `S6`）或大段 evidence 失败时，基于章节骨架与修复合同重建整章。
...
- 优先修复结构问题，重建完整正文。
- 若修复 `S6`，必须保持预期差可证伪；short / deep 的第 08 章若没有独立观点，
  写"暂无可证伪预期差"，并将研究动作降为"观察 / 继续验证"，不得给买卖建议。
